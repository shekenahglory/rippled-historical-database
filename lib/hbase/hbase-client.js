  ...
  if (options.live) {
    ...
    self.getRow({
        ...
        .nodeify(function(err, rate) {
          ...
          rates[data.startTime || data.time] = rate;
...
HbaseClient.prototype.getCapitalization = function (options, callback) {
  ...
  if (options.interval && options.interval !== 'day') {
    if (options.interval === 'week') {
      ...
      // NOTE: not using isoWeek because
      // we later offset by 1 day
...
HbaseClient.prototype.getAccountBalanceChanges = function(options, callback) {
  ...
  function formatChanges(rows) {
    rows.forEach(function(row, i) {
      ...
      rows[i].tx_index = Number(row.tx_index);
      rows[i].executed_time = Number(row.executed_time);
      rows[i].ledger_index = Number(row.ledger_index);

      // node_index can be null
      if (row.node_index) {
        rows[i].node_index = Number(row.node_index);
      } else {
...
HbaseClient.prototype.getExchanges = function (options, callback) {
  ...
  */
  ...
  function reduce (rows) {
    ...
    rows.forEach(function(row) {
      if (options.base.currency    === 'XRP' && row.base_amount < 0.0005) return;
      if (options.counter.currency === 'XRP' && row.counter_amount < 0.0005) return;
